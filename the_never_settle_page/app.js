...
buyButton.addEventListener('click', () => {
  ...
  if(cartIconStyle.animationPlayState == 'running') {
    ...
    if(sizeButtonsArray[0].classList.contains('chosen')) {
      loop(nameOfProduct, 'M', b, '../assets/batlle creek_300', subName, prices[0]);
    }
    else if(sizeButtonsArray[1].classList.contains('chosen')) {
      loop(nameOfProduct,'L', b, '../assets/batlle creek_300', subName, prices[1]);
    }
    else {
      loop(nameOfProduct,'XL', b, '../assets/batlle creek_300', subName, prices[2]);
    }
    quantity = 0;
    result.innerHTML = 0;
  }
