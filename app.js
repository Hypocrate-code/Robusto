...
function loop(name, size, qtt, productImg) {
  ...
  for(i=0;i<localLength;i++) {
    ...
    if(ourProduct.size === tableau.size) {
      let initQuantity = parseInt(JSON.parse(localStorage[i]).quantity) + parseInt(qtt);
      localStorage.setItem(i, JSON.stringify({name:name, size:size, quantity:initQuantity, link:productImg}));
      return;
    }
  }
  localStorage.setItem(localLength, JSON.stringify(ourProduct));
};
...
buyButton.addEventListener('click', () => {
  ...
  if(cartIconStyle.animationPlayState == 'running') {
    ...
    if(sizeButtonsArray[0].classList.contains('chosen')) {
      loop(nameOfProduct, 'M', b, 'Images/batlle creek.jpg');
    }
    else if(sizeButtonsArray[1].classList.contains('chosen')) {
      loop(nameOfProduct,'L', b, 'Images/batlle creek.jpg');
    }
    else {
      loop(nameOfProduct,'XL', b, 'Images/batlle creek.jpg');
    }
    quantity = 0;
    result.innerHTML = 0;
  }
